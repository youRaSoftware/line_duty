...
- Feature root in `features/lib/` (`game/`, `menu/`, …); for `game` also `engine/` (Flame / )
...
- Shared UI in `core_ui/lib/src/` (cross-check names against `.claude/shared/line_duty_ui_reference.md`)
- Spec: `.claude/my_docs/SPEC.md` (which frames / rules the feature implements)

### Step 2: Scan bottom-up

**Domain:** every model field with type and default, `.empty()` values, `copyWith`, getters with business logic; enums (`LaneColor`: `mergeScore`, `emoji`, `next`); repository interfaces (method signatures).
...
**Engine (game only):** `Line DutyGame` — world size, gravity, walls, input handling (drag / tap), drop cooldown, deadline logic, cubit calls (`onDropped`, `onMerge`, `gameOver`), `reset()`, `paused` handling; `UnitComponent` — radius from `AppDimens.ballRadii`, fixture params, rendering, merge detection.
