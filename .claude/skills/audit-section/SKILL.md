---
name: audit-section
description: "Line Duty Flutter project skill for a full pre-release audit of a feature across all layers: vertical analysis (Hive provider → RepositoryImpl → Domain Model → Cubit → State → Form/Widgets, plus the Flame engine for the game) PLUS horizontal analysis (duplicate logic, dead code, suboptimal patterns, architecture questions, spec conformance against .claude/my_docs/SPEC.md). Triggers when user asks to: audit a section, аудит раздела / фичи, проверь перед релизом, pre-release check, полный аудит, проверь поток данных, check gameplay rules against the spec. User provides: feature path, spec/doc paths, optionally an output path."
...
3. **Documentation** — `.claude/my_docs/SPEC.md` (always) + any feature doc from `analyze-feature`
4. **UI reference** — always `.claude/shared/line_duty_ui_reference.md`
5. **Output path** (optional) — default `.claude/my_docs/{section_name}_audit.md`

Ask for missing inputs before starting.

## The Line Duty vertical chain
...
[game only] Line DutyGame () ⇄ GameCubit: engine calls onDropped / onMerge / gameOver; form sets paused / reset()
...
Read all documentation and the UI reference. Build the expected behaviour: screens (SPEC.md § 2), gameplay rules (§ 3: radii, weights 5:4:3:2:1, cooldown 450 ms, deadline 1.5 s, score `2^tier`, t11 jackpot), tokens (§ 1).
