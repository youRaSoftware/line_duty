...
- Номера кадров мокапа (`.claude/my_docs/SPEC.md` § 2) или скриншот
...
- Пути к документации (ТЗ — `.claude/my_docs/SPEC.md`, документы из `analyze-feature`)
...
Документация: .claude/my_docs/SPEC.md
...
├── shared/
│   └── line_duty_ui_reference.md   ← UI/архитектурный референс (токены, виджеты, стиль кода) — его читают create-* скиллы
├── my_docs/                      ← ТЗ (SPEC.md), процесс релиза, документы из analyze-feature / audit-section
...
A: `create-*`, `analyze-feature` и `audit-section` обязаны прочитать `.claude/shared/line_duty_ui_reference.md` — это источник правды по токенам, виджетам и стилю кода. Если меняете `AppColors` / `AppFonts` / `AppDimens` или добавляете виджет в `core_ui` — обновите и референс.
