...
**Название (Name) во всех локалях: `Line Duty: Route Arcade`** (23 символа, лимит 30). Под иконкой — Line Duty. Просто «Line Duty» в русской локализации App Store Connect занято другим приложением (имя проверяется по каждой локализации отдельно, освободить можно только по товарному знаку), поэтому имя с суффиксом ставится и в английскую, и во все остальные. Если «Line Duty: Route Arcade» в какой-то локали тоже окажется занято — запасное локализованное имя:

| Локаль | Запасное имя |
|---|---|
| en-US | Line Duty: Draw & Dispatch (26) |
| ru | Line Duty: аркада-диспетчер (27) |
| de-DE | Line Duty: Routen-Arcade (24) |
| es-ES | Line Duty: arcade de rutas (26) |
| fr-FR | Line Duty: arcade de routes (27) |
| pt-BR | Line Duty: arcade de rotas (26) |
| ja | Line Duty: ルートアーケード (19) |

Слова из названия и подзаголовка в keywords не повторяются — они и так индексируются.
