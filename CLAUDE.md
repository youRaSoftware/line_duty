...
- Линия ворот (верх ворот): фигура судится, когда её **центр** пересёк линию (не край хитбокса), а пристыкованная к своим воротам с недоеденным маршрутом не судится вовсе — маршрут кончается на входе; на линии фигура **сначала проверяется на свои ворота** — задевает их краем (центр не дальше полуширины ворот + `gateCatchSlack` = радиус фигуры) → доставка (+`GameRules.scorePerDelivery`, вспышка ворот), даже если ближайшие по центру чужие (зоны захвата соседей перекрываются на ~12 ед., спорная полоса в пользу игрока). Иначе — конец забега как «Не те ворота». Столкновение двух фигур — конец забега «Столкновение». В обоих случаях поле замирает (`frozen`), играет вспышка, через `crashFreeze` движок зовёт `GameListener.onCrash`. `clearCrash()` (продолжение) убирает разбитые и всё в радиусе; `reset()` — новый забег.
